...
## 2026-08-09 — Step 6's three-primary regression (ADR-121 addendum)

- Fixed: **Step 6 was back to stacking three loud primaries** — `#btn-deal-cut`, `#btn-complete-next`
  ("Try another problem"), and `#btn-finish` ("Finish lesson") — the exact defect ADR-121
  ...
  solving)` line — the ADR-121 fix itself — while leaving `btn--primary` hardcoded on
  ...
  `verify/interaction.mjs` (§8, ADR-121's oracle) once re-run, and reproduced live: 3 primaries at
  Step 6 in free play.
- Fixed: restoring the deleted toggle alone was not enough — `#btn-finish` is a genuinely new
  primary ADR-121 never accounted for, so putting the old line back verbatim still leaves 2 loud
  ...
  `#btn-complete-next` while a problem is loaded (mid-problem or already solved, matching ADR-121's
...
- Extended: `verify/interaction.mjs` §8 now drives a full load → Step 6 → exit cycle and asserts the
  accent at each stage, not just the free-play case ADR-121 originally covered.
- See `DECISIONS.md` ADR-142 for why this fix's decision record lives here rather than under a
  fresh root `ADR-121` body: ADR-121 is one of the 45 references ADR-142 documents as orphaned from
