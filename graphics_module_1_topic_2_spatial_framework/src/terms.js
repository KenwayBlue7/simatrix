...
// quadrant, rabatment — is marked up in the step copy (spatialSteps.js) as a
...
// The placement behaviour is ported from the master Module2/src/terms.js: the
// popover is `position: fixed` so it escapes the step card's `overflow-y: auto`
// clip, opens below the term, flips above when there is not enough room, is
// clamped to the viewport horizontally, dismisses on Escape, and re-places on
// scroll so it tracks the term instead of drifting.
//
// Layering (ADR-007 / RULES.md §3.6): leaf module. main.js calls initTerms()
// once at startup. It imports spatialSteps.js the same pure-data way stepper.js
// does; it never touches the orchestrator or another behaviour leaf.

import { TERMS } from './spatialSteps.js';
