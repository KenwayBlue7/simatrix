...
//
// EXTRACTED from Topic 2.1 (which already has the 'circle' and 'curve' kinds — a sampled
// polyline for a traced locus, drawn as a single <path> of 'L' segments) plus ONE new
// addition this topic needs: the 'curve' kind's `dashed` flag, independent of `role` — a
// helix's front-view projection continuously alternates visible/hidden as it winds behind
// the cylinder, the standard hidden-line convention, which is a different axis from
// role's given/move/result meaning (this topic's own CLAUDE.md records the reasoning).
//
...
/** Cumulative polyline length — used to size the stroke-dashoffset draw-on exactly like a
 *  straight line's, so a traced curve "draws itself" at the same visual rate. */
...
/** How long a step's draw-on takes — a sweeping arc, a drawn line, or a traced curve needs
 *  enough time to actually watch it happen; a point/label snapping into place doesn't. A
 *  dashed (hidden-line) curve segment fades in like a point, it doesn't draw on. */
function durationFor(step) {
  if (step.kind === 'curve' && step.dashed) return 600;
  return step.kind === 'arc' || step.kind === 'line' || step.kind === 'curve' ? 1800 : 600;
}
...
function buildStepNode(step) {
  ...
  if (step.kind === 'circle') {
    // A full given circle (base circle / rolling circle). Native <circle>, not an arc
    // path — an SVG elliptical-arc command can't express a full 360° sweep (start/end
    // points coincide).
  ...
  if (step.kind === 'curve') {
    // A sampled polyline — a traced curve (roulette, spiral, or this topic's helix) is a
    // genuine locus, not a compass-constructible line/arc/circle.
    const { points, role, dashed } = step;
    // New this topic: `dashed` (independent of `role`) — a helix's front-view projection
    // continuously alternates between the near/visible half-turn and the far/hidden
    // half-turn (occluded by the cylinder itself), the standard hidden-line convention
    // (RULES §4 first-angle citation) — a DIFFERENT axis from role's given/move/result
    // meaning, so it's its own flag, not a fourth role. Hidden segments fade in statically
    // (a static hidden-line dash pattern, like a 'point'/'dim' step) rather than drawing
    // on progressively — hidden-line convention is about revealing structure, not miming a
    // drafting tool's motion, so the progressive stroke-dashoffset reveal (correct for the
    // solid/visible curve) would be the wrong metaphor here.
    if (dashed) {
      const node = el('path', {
    ...
    const reveal = (t) => { node.setAttribute('stroke-dashoffset', String(len * (1 - t))); };
    const finalize = () => { node.removeAttribute('stroke-dasharray'); node.removeAttribute('stroke-dashoffset'); };
    return { node, reveal, finalize };
  }
