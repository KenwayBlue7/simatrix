...
const P_RADIUS = 0.12;     // the ink sphere for P — kept small so the point reads precise
...
// (P_RADIUS 0.12, halo ~0.19 — 0.20 clears the halo edge). This sidesteps a
 ...
 * @param {boolean} [opts.showHP=false]  Draw the TOP VIEW layer — p on HP, its
 *   in-plane connector, and the dashed P→p projector (revealed on the From-HP
 *   step; the legacy Points lesson's showHP flag).
 * @param {boolean} [opts.showVP=false]  Draw the FRONT VIEW layer — p′ on VP,
 *   its in-plane connector, and the dashed P→p′ projector (revealed on the
 *   From-VP step; the legacy showVP flag). The two layers are independent so
 *   the guided sequence can teach one shadow at a time.
...
export function createPointRig({
  position, showHP = false, showVP = false,
