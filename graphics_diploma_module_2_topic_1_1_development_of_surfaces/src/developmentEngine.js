// Development-of-surfaces engine — INDEPENDENT COPY (ADR-112) of the KTU-track engine at
// ../graphics_module_3_topic_2_development_of_surfaces/src/developmentEngine.js (ADR-066).
// NOT a byte-identical shared file under RULES §1.3/§1.4 — this topic's copy is free to
// drift (different ShapeType table, different scope), so fix bugs here directly; there is
// no obligation to backport into the KTU original or vice versa.
//
// What THIS topic actually calls (per ADR-112 §2): parallelLayout()'s cylinder branch and
// computeCutDistances()'s parallel/cylinder branch, as pure geometry calculators feeding
// constructions.js's step-list — NOT the draw*() functions below (drawDevelopment /
// drawParallelDevelopment / drawRadialDevelopment / drawStringPath), which paint directly
// and non-animated. This topic's whole pedagogy is animated step-by-step draw-on
// (renderConstruction.js), so those direct-paint functions are unused here; kept rather
// than deleted for audit trail back to the source engine, in case a later "final
// assembled pattern" snapshot view wants them. Radial-line math (radialConeLayout /
// radialPyramidLayout) and the string-path/"ant" geodesic functions are also unused — this
// topic's scope is rectangular prism, cylinder, and a two-piece symmetric 90° elbow only,
// no pyramid/cone/shortest-path problems (see root ../DECISIONS.md ADR-112).
//
// The rectangular prism (unequal base sides L≠W, unlike this file's equal-side
// PRISM_SIDES table below) is NOT run through parallelLayout() — it gets its own
// stretch-out calc in constructions.js.
//
// Original KTU header, kept for context on the parts still in active use:
//
...
//
// Elbow reuse (ADR-112): computeCutDistances()'s parallel/cylinder branch — a single
// localPlane cutting a plain cylinder — is exactly what a 90° two-piece elbow's per-piece
// truncation needs (Bhatt Fig. 15-10 / K.C. John Fig. 15.12 Ex. 15.9's single-cut cylinder
// math). constructions.js calls it once per piece with a 45°-inclined localPlane and
// mirrors the result for the second piece — no new math.
