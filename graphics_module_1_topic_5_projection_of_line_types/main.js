...
// renderer — see DECISIONS.md), the perspective + orthographic cameras + their
 ...
 *  a shorter distance. Target lifted so the subject line centres. This is only the transient
 *  pre-Step-1 pose — STEPS[0].cam (lineTypesData.js) is what frameStep glides to and settles on,
 *  scaled by the same factor so boot doesn't visibly jump. */
 ...
 *  frameTween (the quick-view / step-vantage movers) so no two moves fight over one tween slot. */
let autoZoomTween = null;

/** True while a step's frameStep() vantage glide is queued or in flight — reframeIfClipped()
 *  skips itself until the glide lands, then re-checks (see frameStep's onComplete). Without this,
 *  rebuild()'s auto-zoom would start dollying from the OUTGOING pose the instant applyStep() calls
 *  it, a heartbeat before frameStep fires its own glide from that same outgoing pose — two movers
 *  racing to different destinations. */
let stepFraming = false;
...
let conDock = null;                          // the floating construction-launcher dock (bottom-right of the 2D panel)
...
const WORKBENCH_CONTROLS = ['tl', 'theta', 'phi', 'rotation']; // union — exitWorkbench() returns all
// Rail clustering (mirrors topic_6's WORKBENCH_GROUPS pattern): breaks the flat 3-wide driver row
// into two titled .dock__group clusters so the rail scans instead of reading as one wall.
const WORKBENCH_GROUPS = [
  { id: 'dimensions',  title: 'Dimensions',  keys: ['tl'] },
  { id: 'inclination', title: 'Inclination', keys: ['theta', 'phi'] },
];
// The Rotation Method launcher docks separately, at the 2D drawing panel's bottom-right corner
// (#con-dock), not inside a rail cluster — see ensureConDock()/enterWorkbench() below.
const CON_DOCK_CONTROLS = ['rotation'];
...
function buildScene(container) {
  ...
  // wins the tail. The FOLD tween is left alone (orbiting during the rabatment is fine). Also
  // cancels any in-flight auto-zoom dolly (ADR-014) — a manual grab is the last word. A cancelled
  // frameStep glide never reaches its onComplete, so stepFraming is cleared here too (else
  // auto-zoom would stay wedged off for the rest of the step).
...
function rebuild() {
  ...
  lineRig = createLineTypeRig({ resolved, view: currentView, foldAngle, width: size3d.x, height: size3d.y });
...
function reframeIfClipped() {
  ...
  // camera move, or the folded flat sheet owns the camera instead (RULES.md §5.10). While a
  // step's own frameStep() vantage glide is queued/in flight, defer — frameStep re-checks via
  // this same function once its glide lands (see frameStep's onComplete).
  if (activeCamera !== camera || folded || foldTween || cameraTween || stepFraming) return;
...
function frameStep(cam) {
  if (!cam) { stepFraming = false; return; } // no vantage queued after all — auto-zoom may resume
  frameTween?.cancel(); frameTween = null;
  autoZoomTween?.cancel(); autoZoomTween = null; // a step vantage is the last word over any push-back
  ...
  if (prefersReducedMotion) {
    camera.position.copy(toPos); controls.target.copy(toTgt); controls.update();
    stepFraming = false; reframeIfClipped(); // re-check now the vantage has landed (ADR-014)
    return;
  }
  const fromPos = camera.position.clone();
  ...
  frameTween = tween({
    ...
    onComplete: () => {
      frameTween = null;
      stepFraming = false;
      reframeIfClipped(); // the vantage settled — push back now if it still clips (ADR-014)
    },
  });
}
...
const simController = {
  ...
  applyStep(meta) {
    ...
    // A step vantage is about to be queued (frameStep, below) — defer rebuild()'s own auto-zoom
    // (ADR-014) until that glide lands, so the two movers never race to different destinations
    // from the same outgoing pose. frameStep clears this once its glide settles (or immediately
    // if meta.cam is absent).
    stepFraming = true;
    rebuild();
...
function ensureWorkbenchRail() {
  ...
  // own .dock__group / .dock__group-title convention (ported from topic_6's rail), reused here
 ...
 *  re-parent the existing rotation [data-ctrl] wrapper into and out of it. */
...
function enterWorkbench() {
  ...
  for (const grp of WORKBENCH_GROUPS) {
    ...
    for (const key of grp.keys) {
      ...
      if (wrap && body) { wrap.hidden = false; body.appendChild(wrap); } // all three drivers live at once, clustered
    }
  }
  // The Rotation Method launcher docks separately, floating at the 2D panel's bottom-right
...
/** Restore the floating layout: hand the drivers + Rotation Method launcher back to #controls
...
/** The launcher's own label swaps to "Replay Rotation Method" once triggered, and back to its
 *  base text on teardown — there is no separate Replay button (ported from topic_6's
 *  tl-replay/trace-replay merge). Rewrites only the .btn__label span so the icon span is
 *  untouched. */
const CON_LABELS = {
  'btn-rotation': { base: 'Rotation Method — True Length', replay: 'Replay Rotation Method' },
};
...
/** A construction needs the sheet visible. The launcher + Replay buttons live inside the
 *  [data-ctrl="rotation"] wrapper, which is now part of WORKBENCH_CONTROLS — it re-parents
 *  into the rail on split entry, so the expanded 50/50 split stays fully usable while the
 *  Rotation Method runs (no more forced demotion to the compact card). */
...
function teardownCon() {
  ...
  setConLabel('btn-rotation', false);
  conMode = null;
}
...
const enterRotation = () => enterCon('rotation', (r) => createRotationMethod({ resolved: r }), 'btn-rotation');

/** The launcher does double duty: first click builds + plays the construction; a click while
 *  it's already active replays the same animation from t=0 (the old separate Replay button's
 *  behaviour, now folded into the launcher itself — a repeat click no longer closes the
 *  construction). It still tears down via the other existing teardownCon() paths (a parameter
 *  edit, a step change, the fold) — there is no longer a click-to-close affordance here. */
function setupConstructions() {
  document.getElementById('btn-rotation')?.addEventListener('click', () => (conMode === 'rotation' ? runCon(conLeaf?.duration ?? 0) : enterRotation()));
}
