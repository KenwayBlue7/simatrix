// First-run orientation (onboard) — the line is on stage from Step 1, so the
// Module-2 empty-state "add a solid" overlay is GONE. What remains are the two
// quiet aids, all dismissible and shown at most once:
//   1. Orbit hint — a one-time, dismissible "Drag to rotate" chip shown on the
//      first step (steps declare orbitHint in lineTypesData.js; the stepper calls
...
//      two views, the point-view collapse, foreshortening, and the true-length
//      rotation the FIRST time each step reveals them. A
...
//      fires first on step 1, and the first spotlight follows once it is
...
/**
 * Contextual spotlight copy + colour tone, keyed by step reveal moment. The tone
 ...
 * AND label (Two-Cue Rule). Step → id mapping lives in lineTypesData.js.
 */
const SPOTLIGHTS = {
  'two-views':   { tone: 'ink', text: 'Two views per line — a front view on the VP and a top view on the HP.' },
  'point-view':  { tone: 'hp',  text: 'A projection collapses to a point when the line points straight at that plane.' },
  'foreshorten': { tone: 'hp',  text: 'Tilt the line and one view shrinks below true length (foreshortened).' },
  'true-length': { tone: 'ink', text: 'Inclined to both planes — both views shorten. Rotate one flat to recover True Length.' },
};
