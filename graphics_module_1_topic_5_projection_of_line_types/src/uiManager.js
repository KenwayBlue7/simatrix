...
// the Module-2 pattern. This topic's dock is three drivers — True Length and the
// two inclinations θ (with HP) and φ (with VP) — plus the guarded ghost-Reset
// confirm. Which of the three a step reveals is set per step in lineTypesData.js.
//
...
export function initUIManager(sim) {
  ...
  let wasInvalid = false; // tracks θ+φ>90 so the constraint is announced once per transition
  ...
  function sync() {
    ...
    // θ + φ must stay ≤ 90° for a real line (resolveLine `valid` flag). The note is
    // visual-only (aria-hidden); the single #sim-status channel narrates the constraint
    // once, on the transition into invalid — not on every commit while it stays invalid.
    if (noteValid) {
      ...
      if (invalid && !wasInvalid) {
        sim.announce('θ plus φ must stay at or below 90 degrees for a real line. Reduce one angle.');
      }
      wasInvalid = invalid;
    }
  }
