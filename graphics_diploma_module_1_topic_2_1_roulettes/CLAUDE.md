...
> 2 ("Miscellaneous Curves", ADR-096) — subtopic 2.1, the first of four (2.2 Spiral Curves, 2.3
> Helix, 2.4 Involutes — the latter split out of this topic per ADR-150). Topic 2 sits **beside**
> Topic 1 ("Geometrical Constructions") inside the
...
Nine roulette curves, taught one at a time: cycloid, superior/inferior trochoid, epicycloid,
superior/inferior epitrochoid, hypocycloid, superior/inferior hypotrochoid. (Involute of a circle
and of a polygon — this topic's former Generator B — split out into its own Topic 2.4,
`../graphics_diploma_module_1_topic_2_4_involutes`, per ADR-150: an involute is generated by a
taut string/line unwinding, a structurally different mechanism from a circle rolling on a base,
and never belonged in the same generator family as the curves below.) **Duplicated from Topic 1.1**
...
## The generator shape — one shared function, not nine hardcoded curves

**The roulette family** (`rollingCurvePoint()` / `contactPoint()` / `rollingCircleCenter()` in
`src/constructions.js`), covering all 9 picker entries: cycloid, superior/inferior trochoid,
epicycloid, superior/inferior epitrochoid, hypocycloid, superior/inferior hypotrochoid. These are
genuinely one equation family — a point at signed distance `offset` from a circle of radius
`rollRadius` rolling without slipping on either a straight line or a fixed circle of radius
`baseRadius` — differing only in those three inputs plus which side of the base circle it rolls
on (`baseType: 'line' | 'outside-circle' | 'inside-circle'`). Hardcoding nine near-identical trig
blocks would be pure duplication that drifts (RULES §1.3/§1.4's shared-logic concern, applied
within this one topic's own file, not just across topic folders). The 9 CONSTRUCTIONS entries are
thin config wrappers around this one function, each exposing only its own textbook-relevant
`given[]`.

**Involute of a circle/polygon — split out (ADR-150).** This topic originally also carried a
second generator (`involuteCirclePoint()`/`involutePolygonArcs()`) deliberately kept separate from
the roulette family above — an involute is a taut string unwinding, not a circle rolling on a
base. That separateness is now a topic boundary too: the involute constructions moved to
`../graphics_diploma_module_1_topic_2_4_involutes`, duplicating (not importing) every primitive
they still need from this file, per RULES §1.4.

**`tangentNormalSteps()`:** the classical fact that a roulette's normal at its traced point always
passes through the curve's instantaneous point of contact with its base. This is why tangent/
normal construction is one routine, not reimplemented per curve — verified against the specific
worked method the source text names for the cycloid (Example 7.1: an auxiliary arc of the
rolling-circle's own radius from the traced point M, cutting the rolling circle's centre-locus at
N, then NC perpendicular to the base — `showClassicalArc` in `tangentNormalSteps()` renders
exactly that sequence for the cycloid construction specifically; every other curve draws the same
MC normal fact more directly, since the auxiliary-arc's radius only equals the rolling-circle
radius in the on-rim cycloid case). Topic 2.4's involute constructions call this same routine too
(from their own duplicated copy) — the shared classical fact holds for an involute's string just
as much as a roulette's rolling circle.

**Why this is 9 picker entries, not fewer:** each named curve keeps its own `id`, `principle` text
(the "why it works" shown on Verify), and its own scoped `given[]` — a superior trochoid problem
shouldn't expose a base-circle radius slider it has no use for. The picker still reads as 9
distinct things to a student; only the geometry underneath is shared.
...
| `src/terms.js` (data) | INFERRED | New `TERMS`: roulette, cycloid, trochoid, epicycloid, hypocycloid, generating circle, base circle |
...
| `src/constructions.js` | INFERRED (new) | Entirely new geometry — the one shared generator described above |
...
Unchanged from every prior topic — Choose (a 9-item picker) → Given (measurements + the Roll
...
└── src/
    ...
    ├── stepper.js                     ← guided-step controller (4-step shape + 9-item picker)
    ...
    ├── constructions.js                           ← the 9 curves' pure geometry (1 shared generator)
