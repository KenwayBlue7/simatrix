...
const WORKBENCH_CONTROLS = ['given', 'guess', 'truelength', 'traces'];
// Rail clustering (topic-local visual grouping, not a platform convention): REVERSE TOPIC has
// only two [data-ctrl] wrappers left to dock (given, guess — no per-field sliders), so a single
// titled cluster replaces Topic 6's two (dimensions / inclination split, which doesn't apply
// once there's nothing to dial).
const WORKBENCH_GROUPS = [
  { id: 'solve', title: 'Given & your guess', keys: ['given', 'guess'] },
];
...
// --- REVERSE TOPIC state (graphics_module_1_topic_7): which shapeData/derived fields the
// active problem hides (the learner must guess them) and what the learner has typed so far.
// This is a SEPARATE state space from currentData — currentData always holds the CORRECT
// values (so the drawing renders right); `guesses` holds the learner's own numbers, which
// start empty and are compared to the problem's `target` by problemLibrary.js. Neither array
// is ever read by rebuild() to CHANGE geometry — only to decide which on-screen labels to hide.
let givenFields = [];
let askFields = [];
let guesses = {};
// htDist/vtDist aren't part of currentData (they're a derived trace position, not a driving
// shapeData param) — when one of them is a GIVEN field, its number arrives out-of-band via
// setFieldGating's third argument, since it can't be read off currentData like TL/theta/phi/aHP/aVP.
let givenExtra = {};
...
/** Whether the learner has typed a guess — gates the Problem Library's "loading clears your
 *  work" confirm. REVERSE TOPIC: unlike Topic 6, currentData changes the instant a problem
 *  loads (it's locked to the answer, not dialled), so "work" means a non-empty guess, not a
 *  currentData diff from defaults. */
function hasDialedWork() {
  return Object.values(guesses).some((v) => typeof v === 'number' && Number.isFinite(v));
}
...
function rebuild() {
  ...
  // REVERSE TOPIC: askFields names which numbers the active problem hides — lineRig.js /
  // compareSheet.js suppress the matching on-screen dimension label so the answer never
  // leaks before the learner guesses it. Empty outside a loaded problem (nothing hidden).
  const hidden = askFields.length ? new Set(askFields) : null;
  ...
  lineRig = createLineRig({ resolved, view: currentView, foldAngle, width: size3d.x, height: size3d.y, hiddenFields: hidden });
  ...
  if (compareOpen && compareSheet) compareSheet.setData(resolved, currentView, hidden);
...
const simController = {
  ...
  /** The full solved shapeData currently driving the drawing (NOT the learner's guesses —
   *  see getGuesses below). Kept for parity/debugging; problemLibrary.js's self-check reads
   *  getGuesses(), not this. */
  state: () => ({ ...currentData }),
  /** Whether there is a guess to lose — gates the library's "clears your work" confirm. */
  ...
  /** The first revealed guess input — focused after a problem loads so a keyboard learner
   *  lands on a field to answer, not the closed overlay. */
  firstControl: () => document.querySelector('#guess-panel input'),

  // --- REVERSE TOPIC seam (graphics_module_1_topic_7) ---------------------------------------
  /** problemLibrary.js calls this right after commit(problem.shapeData) — tells main.js which
   *  fields to hide on the drawing (askFields) and which to leave visible read-only
   *  (givenFields), then triggers a rebuild so lineRig.js/compareSheet.js pick up the change,
   *  and asks uiManager to redraw the given/guess panels. */
  setFieldGating(given, ask, extra) {
    givenFields = given ?? [];
    askFields = ask ?? [];
    givenExtra = extra ?? {};
    rebuild();
  ...
  getFieldGating: () => ({ given: givenFields, ask: askFields }),
  /** A GIVEN field's display value — TL/theta/phi/aHP/aVP read straight off currentData;
   *  htDist/vtDist (not part of shapeData) read from the out-of-band givenExtra passed to
   *  setFieldGating. null if the field isn't actually given right now. */
  getGivenValue(key) {
    if (!givenFields.includes(key)) return null;
    if (key in currentData) return currentData[key];
    return givenExtra[key] ?? null;
  },
  /** The learner's typed guesses — a SEPARATE state space from currentData/shapeData. */
  getGuesses: () => ({ ...guesses }),
  /** uiManager.js calls this on every guess-input change. Lightweight: no rebuild (geometry
   *  never depends on a guess), just re-runs the self-check via the same onStateChange seam. */
  setGuess(key, value) {
    guesses = { ...guesses, [key]: value };
    notifyStateChange();
  },
  clearGuesses() { guesses = {}; notifyStateChange(); },
...
const compare = {
  show() {
    ...
    if (compareSheet) compareSheet.setData(resolveLine(currentData), currentView, askFields.length ? new Set(askFields) : null);
    updateCompareChip();
...
// REVERSE TOPIC: constructions launched pre-check must suppress the same askFields digits
// the base rig/sheet already hide (main.js's `hidden` Set, built once per rebuild() above) —
// otherwise clicking a launcher mid-guess would replay the correct number baked into its
// label. Re-read fresh here (not the stale `hidden` local from rebuild()'s closure) so a
// problem switch between rebuilds still gates the launcher correctly.
const conHidden = () => (askFields.length ? new Set(askFields) : null);
const enterTrace = () => enterCon('trace', (r) => createTraces({ resolved: r, hiddenFields: conHidden() }), 'btn-traces');
...
function init() {
  ...
  try {
    ...
    // The textbook Problem Library (ADR-015) — AFTER the stepper, onboarding (cueHint), and the
    // dock exist, so its facade calls all resolve. Data-driven from lineProblems.js. REVERSE
    // TOPIC: no entryStep — there is only one step (the given drawing + guess panel are both
    // always visible once a problem is active), so there is nothing to route to.
