# Changelog — Finding Angles & True Length

## 2026-08-05 — Topic created: reverse Lines problems (cloned from Topic 6, ADR-009)
- Added: `graphics_module_1_topic_7_reverse_line_problems`, a full copy-and-simplify clone of
  `graphics_module_1_topic_6_projection_of_straight_lines` (ADR-009 discipline — manual copy, no
  shared library), inverting the interaction: the drawing is shown FINISHED and LOCKED (not
  learner-dialled), and the learner guesses one or more of TL / θ / φ / H.T. / V.T. instead.
  `anim.js`, `lineRig.js` (geometry), `sheet2DLayout.js`, `dimensions.js`, `labels.js`/`labels/`,
  `terms.js`, `compareSheet.js` (geometry), `trueLength.js`, `traces.js` carried over unchanged or
  near-unchanged (see below for the two label-suppression edits).
- Added: `src/lineProblems.js` — 13 problems from N.D. Bhatt's "Projection of Straight Lines"
  (Chapter 10), each with a `shapeData` (the full solved answer, drives the drawing),
  `givenFields`/`askFields` (which of TL/theta/phi/aHP/aVP/htDist/vtDist are shown vs guessed),
  and `target`. Verified against the book's own printed answer where one exists (10-33: θ=45°,
  φ=30°, TL=100mm, reproduced exactly). Two problems (10-5, 10-6) are the book's own method demos
  with no printed numeric data in this edition — supplied consistent instances of the same
  procedure, noted per-problem. 10-26/10-27/10-28 (portion-in-quadrant problems) deliberately
  deferred to a future "advanced" tier — a different target concept (segment TL, not the line's
  own TL) not yet modelled here.
- Changed: `src/problemLibrary.js` — self-check now compares a NEW, separate `guesses` state
  (never pre-filled) against `problem.target`, over `problem.askFields` only, instead of Topic
  6's live-dialled-shapeData-vs-target compare. `loadProblem()` now LOCKS the drawing
  (`sim.commit(problem.shapeData)`) instead of routing to a dial-able step.
- Changed: `src/lineRig.js` / `src/compareSheet.js` — new optional `hiddenFields` param
  suppresses the numeric TL dimension, the θ/φ arc labels, and the "= TL" viewlength suffix for
  whichever fields are the active problem's `askFields` — without this, Topic 6's always-on 3D/2D
  dimension labels would print the answer before the learner guessed it. Geometry itself is
  unaffected (shapeData still carries the correct value — only the printed digit is hidden).
- Changed: `src/uiManager.js` — Topic 6's five-slider dock replaced with two dynamic panels
  (`given-panel` read-only, `guess-panel` bare numeric inputs), built from
  `sim.getFieldGating()`/`sim.getGivenValue()`/`sim.getGuesses()` on every problem load.
- Changed: `src/lineSteps.js` — Topic 6's 5-step build-up wizard collapsed to one step (nothing
  to build up to — the drawing is already finished); `stepper.js` itself is unchanged.
- Known gaps (not yet addressed, flagged rather than silently shipped): the True Length & Angles /
  Traces construction launchers stay unlocked even before a guess is checked — invoking one mid-
  guess replays the correct construction and could trivialize an answer; `onboarding.js` and this
  topic's `CLAUDE.md` architecture prose still describe Topic 6's build-up framing, not yet
  rewritten for reverse framing; not yet runtime-verified in a browser (code-reviewed and
  hand-verified against the textbook's own printed answers only).
