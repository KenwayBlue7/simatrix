...
export function createTraces({ resolved, hiddenFields = null }) {
  // REVERSE TOPIC (graphics_module_1_topic_7): mirrors lineRig.js/trueLength.js's hide() pattern.
  // IMPORTANT ASYMMETRY vs TL/theta/phi: this file has NO printed numeric digit for htDist/vtDist
  // to begin with — the trace's mm distance from xy is never rendered as text anywhere in this
  // construction, only the 'HT'/'VT' letter tag marks the point. The actual value is the MARKER'S
  // POSITION on the sheet's drafting grid (visible, countable in grid cells) — that IS the
  // construction's whole purpose, and hiding it would mean hiding the geometry itself, which is
  // out of scope (geometry must stay visible per the fix directive). So hide('htDist')/hide('vtDist')
  // below only blanks the 'HT'/'VT' identifying letter — a partial, symbolic suppression. The real
  // leak (the point's on-grid position) is NOT closed by this change; flagged for the caller.
  ...
  if (T.method === 'II') {
    ...
    function buildTrapezoidTrace(P, Q, trap, trace, hpOrVp) {
      ...
      const askKey = hpOrVp === 'hp' ? 'htDist' : 'vtDist';
      addLabel(mark, hide(askKey) ? '?' : (hpOrVp === 'hp' ? 'HT' : 'VT'), hpOrVp === 'hp' ? TRACE.ht : TRACE.vt,
    ...
    if (!T.noHT && T.h) {
      ...
      addLabel(htG, hide('htDist') ? '?' : 'HT', TRACE.ht, { color: '--color-hp-line', size: '10px' });
    }
    if (!T.noVT && T.v) {
      ...
      addLabel(vtG, hide('vtDist') ? '?' : 'VT', TRACE.vt, { color: '--color-vp-line', size: '10px' });
    }
