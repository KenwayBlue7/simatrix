 ...
 */
export function createLineRig({ resolved, view, foldAngle = 0, width = 1, height = 1, hiddenFields = null }) {
  // REVERSE TOPIC (graphics_module_1_topic_7): hiddenFields names which of the current problem's
  // askFields are still unguessed — their numeric value must not leak onto the always-visible 3D
  // rig (the TL rod dimension below, and the theta/phi angle labels LabelManager.build() draws).
  // The geometry itself is unaffected: shapeData already carries the CORRECT value (that's what
  // makes the drawing right), only the printed digit is suppressed. null/undefined = show
  // everything (Topic 6's original forward-mode behaviour).
  ...
  if (view.showLine && resolved) {
    ...
    // fvTrue only happens when phi=0 and tvTrue only when theta=0 (the tag IS the announcement
    // "this angle is zero") — suppress each tag when that angle is a hidden askField, same as
    // the TL dimension above.
    const fvTrue = Math.abs(M.fvLen - M.tl) < 0.5 && !hide('phi');
    const tvTrue = Math.abs(M.tvLen - M.tl) < 0.5 && !hide('theta');
    ...
    const tlDim = addOrientedDimension(group, va, vb,
      { color: COL.ink, resolution: res, materials, widthPx: 1.0, gap: 0.1, overshoot: 0.2, arrowLen: 0.4, offsetLen: DIMENSION_OFFSET, value: hide('TL') ? 'TL ?' : `TL ${Math.round(M.tl)}` });
  ...
  labelManager.build({
    view,
    line: linePts,
    // Hidden askFields pass 0 — LabelManager's own `theta > 1 && theta < 89.5` gate then
    // naturally skips the arc label (0 fails that gate), so no separate suppression flag
    // needs to reach LabelManager itself.
    theta: hide('theta') ? 0 : (resolved?.theta ?? 0),
    phi: hide('phi') ? 0 : (resolved?.phi ?? 0),
  });
