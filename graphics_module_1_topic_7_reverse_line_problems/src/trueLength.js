...
export function createTrueLength({ resolved, method = 'I', hiddenFields = null }) {
  // REVERSE TOPIC (graphics_module_1_topic_7): hiddenFields names which of the active problem's
  // askFields are still unguessed — mirrors lineRig.js's hide()/hiddenFields pattern exactly, so
  // launching this construction pre-check doesn't leak the TL/θ/φ digit the base rig already
  // suppresses. Geometry (the rotation arcs, projector lines, locus) is UNAFFECTED — only the
  // printed number in each label's text is swapped for a "?" placeholder. null/undefined = show
  // everything (Topic 6's original behaviour). α/β (the apparent-angle labels below) are NOT
  // askFields themselves, but since they are a monotonic function of the same θ/φ this
  // construction reveals (α ≥ θ, β ≥ φ), BOTH labels are gated together whenever EITHER theta
  // or phi is hidden — closes the back-inference gap without needing to pin down exactly which
  // of α/β pairs with which angle.
  const hide = (key) => hiddenFields != null && hiddenFields.has(key);
  ...
  if (method === 'II') {
    ...
    const thLbl = addLabel(group, hide('theta') ? 'θ=?' : `θ=${Math.round(resolved.theta)}°`, [midTV[0], midTV[1] - tlLift, 0], { color: '--tl-green', size: '11px' });
    const tlLblA = addLabel(group, hide('TL') ? 'TL ?' : `TL ${Math.round(resolved.tl)}`, [midTV[0], midTV[1] - tlLift * 2, 0], { color: '--tl-green', mono: true, size: '10px' });
    const phLbl = addLabel(group, hide('phi') ? 'φ=?' : `φ=${Math.round(resolved.phi)}°`, [midFV[0], midFV[1] + tlLift, 0], { color: '--tl-green', size: '11px' });
    const tlLblB = addLabel(group, hide('TL') ? 'TL ?' : `TL ${Math.round(resolved.tl)}`, [midFV[0], midFV[1] + tlLift * 2, 0], { color: '--tl-green', mono: true, size: '10px' });
    const hideApparent = hide('theta') || hide('phi');
    const betaLbl = addLabel(group, hideApparent ? 'β=?' : `β=${Math.round(resolved.beta)}°`, [midTV[0], midTV[1] - tlLift * 3, 0], { color: '--tl-green', size: '11px' });
    const alphaLbl = addLabel(group, hideApparent ? 'α=?' : `α=${Math.round(resolved.alpha)}°`, [midFV[0], midFV[1] + tlLift * 3, 0], { color: '--tl-green', size: '11px' });
  ...
  const thLbl = addLabel(group, hide('theta') ? 'θ=?' : `θ=${Math.round(resolved.theta)}°`, bisectorAnchor(fvPiv, dirA / 2, R), { color: '--tl-green', size: '11px' });
  const tlLblA = addLabel(group, hide('TL') ? 'TL ?' : `TL ${Math.round(resolved.tl)}`, [(fvPiv[0] + bTLA[0]) / 2, (fvPiv[1] + bTLA[1]) / 2 + tlLift, 0], { color: '--tl-green', mono: true, size: '10px' });
  const phLbl = addLabel(group, hide('phi') ? 'φ=?' : `φ=${Math.round(resolved.phi)}°`, bisectorAnchor(tvPiv, dirB / 2, R), { color: '--tl-green', size: '11px' });
  const tlLblB = addLabel(group, hide('TL') ? 'TL ?' : `TL ${Math.round(resolved.tl)}`, [(tvPiv[0] + bTLB[0]) / 2, (tvPiv[1] + bTLB[1]) / 2 - tlLift, 0], { color: '--tl-green', mono: true, size: '10px' });
  const R2 = { x: R.x * 1.4, y: R.y * 1.4 }; // wider elliptical radius so α/β labels clear θ/φ's
  const hideApparent2 = hide('theta') || hide('phi');
  const alphaLbl = addLabel(group, hideApparent2 ? 'α=?' : `α=${Math.round(resolved.alpha)}°`, bisectorAnchor(fvPiv, startB / 2, R2), { color: '--tl-green', size: '11px' });
  const betaLbl = addLabel(group, hideApparent2 ? 'β=?' : `β=${Math.round(resolved.beta)}°`, bisectorAnchor(tvPiv, startA / 2, R2), { color: '--tl-green', size: '11px' });
