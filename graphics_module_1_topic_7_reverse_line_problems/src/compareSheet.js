...
export function createCompareSheet({ px = 1, py = 1 } = {}) {
  ...
  return {
    ...
    setData(resolved, view, hiddenFields = null) {
      // REVERSE TOPIC (graphics_module_1_topic_7): guard against the same theta=0/phi=0
      // announcement lineRig.js guards (a hidden askField's "= TL" suffix would confirm its
      // angle is exactly zero before the learner guesses). hiddenFields optional — null =
      // Topic 6's original forward-mode behaviour, nothing suppressed.
      ...
      const fvVal = `${Math.round(resolved.fvLen)}${L.fvTrue && !hide('phi') ? ' = TL' : ''}`;
      const tvVal = `${Math.round(resolved.tvLen)}${L.tvTrue && !hide('theta') ? ' = TL' : ''}`;
      ...
      // 2D endpoint letters — the boxed a′/b′ (front view, VP amber, above XY) and a/b (top view,
      // HP teal, below XY) chips, built through the SAME labels.js factory as the 3D scene + XY marks
      // and placed by the SHARED sheet2D.chip policy (labelPlacement.js): lateral spread `out` (a left,
      // b right), a perpendicular `plane` lift off each view line, and a `pointOut` collapse to a single
      // combined chip when a view foreshortens to a point. They live in `group`, so clearGroup() →
      // disposeLabels(group) rebuilds them on every setData (step / θ / φ / TL / fold / reset) with no
      // DOM leak — the identical lifecycle as the XY marks above.
      const C = PLACEMENT.sheet2D.chip;
      if (L.fvPoint) {
        addLabel(group, 'a′b′', [L.A1[0] + C.pointOut, L.A1[1] + C.plane], { chip: true, color: '--color-vp-line' });
      } else {
        addLabel(group, 'a′', [L.A1[0] - C.out, L.A1[1] + C.plane], { chip: true, color: '--color-vp-line' });
        addLabel(group, 'b′', [L.B1[0] + C.out, L.B1[1] + C.plane], { chip: true, color: '--color-vp-line' });
      }
      if (L.tvPoint) {
        addLabel(group, 'ab', [L.A2[0] + C.pointOut, L.A2[1] - C.plane], { chip: true, color: '--color-hp-line' });
      } else {
        addLabel(group, 'a', [L.A2[0] - C.out, L.A2[1] - C.plane], { chip: true, color: '--color-hp-line' });
        addLabel(group, 'b', [L.B2[0] + C.out, L.B2[1] - C.plane], { chip: true, color: '--color-hp-line' });
      }
