// Parameter dock controller — Sections of Solids.
//
// REWRITTEN for this topic. The scaffold shipped the Module 2 dock controller verbatim, but
// none of the markup it wires (rng-base, tgl-fihp, …) — so nothing imported it and the Reset
// button was silently dead, the exact unwired-markup failure ADR-048 recorded on the Glass
// Box topic. This version wires only the controls this topic actually renders:
//
//   Step 1 — the solid picker (#ctl-shape).
//   Step 2 — the section-plane controls: #tgl-section (cut on/off), #ctl-section-orient
//            (which reference plane the cut inclines toward), #rng-sec-angle/#num-sec-angle,
//            #rng-sec-offset/#num-sec-offset.
//   Step 5 — #btn-face-section (camera square-on to the section face).
...
// DECISIONS.md ADR-058), so section edits go through sim.commitSection(), shape edits
// through sim.commit(); both route through the single rebuild() pipeline.
//
 ...
 * @property {() => {enabled:boolean, orientation:'HP'|'VP', angleDeg:number, offset:number}} sectionState
 *   Copy of the topic-local section-plane state.
 * @property {(partial:object) => void} commitSection  Merge params into section state and rebuild().
 * @property {() => void} faceSection                  Tween the camera square-on to the section face.
 ...
 * DISPLAYED unit (mm for the offset, degrees for the angle); `scale` maps display → stored
 * world units (display = stored × scale, so commit divides by scale; 1 world unit = 10 mm,
 * matching the Module 2 dock convention).
 */
const SECTION_SLIDERS = [
  { key: 'angleDeg', range: 'rng-sec-angle',  num: 'num-sec-angle',  min: 0,   max: 90, decimals: 0, unitWord: 'degrees', scale: 1 },
  { key: 'offset',   range: 'rng-sec-offset', num: 'num-sec-offset', min: -40, max: 40, decimals: 0, unitWord: 'mm',      scale: 10 },
];
...
export function initUIManager(sim) {
  ...
  const orientSelect = $('ctl-section-orient');
  ...
  shapeSelect.addEventListener('change', () => {
    ...
    // but the lock keeps the data honest for the generators).
  ...
  tglSection.addEventListener('change', () => {
    ...
    sim.announce(tglSection.checked
      ? 'Section plane on. The solid is cut; the highlighted face is the section.'
      : 'Section plane off. The solid is whole again.');
    syncAll(); // enable/disable the dependent angle + offset controls
  }, listen);

  // --- Section orientation ---
  orientSelect.addEventListener('change', () => {
    sim.commitSection({ orientation: orientSelect.value });
    sim.announce(orientSelect.value === 'VP'
      ? 'Cutting plane perpendicular to the HP, inclined to the VP.'
      : 'Cutting plane perpendicular to the VP, inclined to the HP.');
  }, listen);

  // --- "Face the section" (Step 5) — pure camera navigation; main.js owns the tween and
  //     the no-section guard, so this stays a plain delegate. ---
  $('btn-face-section')?.addEventListener('click', () => sim.faceSection(), listen);
  ...
  function syncAll() {
    ...
    orientSelect.value = sec.orientation;
    orientSelect.disabled = !sec.enabled;
