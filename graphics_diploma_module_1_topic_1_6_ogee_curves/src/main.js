...
//
// New this topic: the draggable handle (constructions.js's `handle` field,
// renderConstruction.js's mountHandle()). Every rebuild(), AFTER the given steps render,
// this file disposes the previous handle and mounts a fresh one for the active
// construction's `handle` descriptor (if it has one — both Ogee constructions do). The
// handle's onCommit callback is the exact same `simController.commit()` every slider in
// uiManager.js already calls — dragging the handle and dragging the matching slider thumb
// are two views of the identical state.params write, so they always stay in sync for free
...
//   -> notify (drives step-gating + the Problem Library self-check). The MOVE/RESULT
//   elements are drawn separately by play() into #dynamic-layer, animated, replayable
//   without re-deriving the given elements or disturbing the handle.
//
...
import { clear, renderStatic, playSteps, computeBounds, mountHandle } from './renderConstruction.js';
...
let activeHandle = null; // mountHandle() handle, disposed/remounted every rebuild()
...
function rebuild() {
  ...
  if (!construction) {
    ...
    if (lastRecipe.handle) {
      const paramSpec = construction.given.find((g) => g.key === lastRecipe.handle.key);
      if (paramSpec) {
        activeHandle = mountHandle(
          svgEl, handleLayer, lastRecipe.handle, state.params[lastRecipe.handle.key], paramSpec,
          {
            onCommit: (value) => simController.commit({ [lastRecipe.handle.key]: value }),
            announce,
          },
        );
      }
