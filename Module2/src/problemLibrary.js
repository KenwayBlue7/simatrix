...
// — against the problem's target with tolerances, so it needs no rotation math and
// face-inclination problems compare cleanly. It is driven by sim.onStateChange, which
...
export function initProblemLibrary(sim) {
  ...
  let hpCueFired = false;    // per-load guard: the round-solid-on-VP HP-depth cue fires once
  ...
  /** Tolerant field compare: shapes/strings by identity, angles to 0.5°, lengths/dists to
   *  0.05. The turn (rotationY) is routed through turnCongruent instead — it is symmetry-aware. */
  function fieldMatches(key, actual, want) {
    if (typeof want !== 'number' || typeof actual !== 'number') return actual === want;
    const angular = key === 'angleHP' || key === 'angleVP' || key === 'rotationY';
    return Math.abs(actual - want) <= (angular ? 0.5 : 0.05);
  }
  ...
  /**
   * True when two turns produce the SAME pose, i.e. they differ by a whole multiple of the
   * base's rotational-symmetry period (within TURN_TOL). A regular base maps onto itself
   * every 360/n°, so e.g. a cube at 45° and at 135° draw identically — accepting only one
   * would mark a correct hand-dialled turn wrong. The spin is applied about the solid's own
   * axis BEFORE any tilt, so congruent turns stay identical even for an inclined solid.
   * Mirror images are NOT congruent and are correctly excluded.
   */
  ...
  /**
   * Fire the floating-chip HP-depth cue once per load, the moment a solid is on the bench,
   * for problems whose target is a ROUND solid (Cylinder/Cone) resting on the VP — the case
   * where students wrongly read the quoted axis height straight onto the HP instead of
   * subtracting the radius to reach the lowest point.
   */
  function maybeCueHpDepth() {
    if (hpCueFired || !activeProblem) return;
    const t = activeProblem.target;
    const round = t.shape === sim.ShapeType.Cylinder || t.shape === sim.ShapeType.Cone;
    if (round && t.restingPlane === 'VP') {
      sim.cueHint('Distance from HP is measured to the lowest point, not the axis — subtract the radius from the quoted axis height.');
      hpCueFired = true;
    }
  }
  ...
  function evaluate() {
    ...
    maybeCueHpDepth(); // solid is now on the bench — fire the per-load HP-depth cue if due
    ...
    for (const [key, want] of Object.entries(target)) {
      ...
      // preset is ON, the last-dialed rotationY no longer drives the solid, so skip it — the
      // modes check below reports any orient mismatch instead of a confusing second diff.
      ...
      // uses the plain tolerant compare (which keeps the 0.5° tolerance for angleHP/angleVP).
      const ok = key === 'rotationY'
        ? turnCongruent(state.rotationY, want, sim.orientPeriod(state.shape))
        : fieldMatches(key, state[key], want);
      if (!ok) diffs.push(FIELD_LABELS[key] ?? key);
    }
    if (target.modes) {
      for (const [mode, want] of Object.entries(target.modes)) {
        // Base orientation accepts EITHER path: the orient-to-corner preset, OR a manual turn
        // dialled to the same pose. When a problem wants the preset ON, treat it as satisfied
        // if the toggle is on, OR the live turn lands on the preset angle (modulo the base's
        // symmetry). want === false stays strict — those problems' preset is a DIFFERENT pose,
        // so the toggle must remain rejected.
  ...
  function loadProblem(problem) {
    ...
    matched = false;
    hpCueFired = false; // re-arm the per-load HP-depth cue for this problem
   ...
   * problem: clear any revealed steps, hide the wrapper, and either offer the button
   * ("Need a hint?") when the problem ships hints, or hide it when it has none.
   */
  function resetHints() {
    hintsShown = 0;
  ...
  /**
   * Reveal the next scaffolded hint step (one per click), announcing it for screen readers.
   * The button advances to "Show next hint (n of N)" while steps remain; once the last is
   * shown it becomes a "Hide hints" toggle (see onHintBtn) so the revealed list can be
   * collapsed to reclaim panel space — the reasoning is scaffolded, never dumped.
   */
