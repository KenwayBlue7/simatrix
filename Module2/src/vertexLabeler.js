// Step-3 annotation layer of the Guided Stepper. Places the standard engineering
// annotations on the solid:
...
function buildLine(positions, color, widthPx, dashed, resolution) {
  ...
  const material = new LineMaterial({
    ...
    transparent: true, // so setOpacity can cross-fade with the solid during the fold
  });
...
/**
 * Wire a Step-3 annotation controller onto a scene. The controller owns one
 * persistent group (added to the scene once) and rebuilds its CSS2DObject labels +
 * LineSegments2 axis/generators on each generate() call. Mirrors the
 * {generate, clear, dispose, setOpacity} lifecycle the projection layer uses, plus
 * setResolution() for the fattened lines, so main.js manages it identically.
 *
...
export function initVertexLabeler(scene, initialResolution) {
  ...
  function generate(mesh) {
    ...
    if (axisLine) group.add(axisLine);
  ...
  /**
   * Fade the annotations (they annotate the 3D solid, so they cross-fade out with
   * it during the flatten). Drives each label's DOM opacity and each line material's
   * opacity, and hides the group once fully transparent so it stops rendering.
   * @param {number} o 0..1
   */
  ...
  return { group, generate, clear, dispose, setOpacity, setResolution };
}
