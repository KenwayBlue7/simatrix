 ...
 * ADR-107 backporting ADR-106) fold in main.js. The observer direction is unchanged by
 * the fold, so this `> 0` test is retained; only the side view's PLACEMENT changed (now
 * beside the FRONT view, not the top view). Re-derive VISUALLY against the worked square
 ...
 * (ADR-107 backporting ADR-106): main.js parents the returned ppGroup under a hinge
 * group NESTED inside vpFoldGroup, translated to local (0, 0, z0), and rotated about
 * its local Y (folding the plane sideways into the VP plane, then riding the VP's own
 * fold down with the front view), so here we draw the bare side view at z = 0 in that
 ...
 *   hinge, z0 − x (ADR-107 backporting ADR-106); harmless at 0 when no profile plane has
 *   been seated yet.
...
export function drawProjections(edgeMap, options = {}) {
  ...
  if (drawConnectors) {
    for (const vertex of uniqueVertices.values()) {
      ...
      // (ADR-107 backporting ADR-106). The PP now hinges SIDEWAYS into the VP plane
      // about the VP∩PP line (+90° about local Y, nested inside vpFoldGroup so it rides
      // the VP's own fold down), carrying a PP point (x, y, 0) at standoff z0 to
      // (−y, 0, z0 − x) — beside the FRONT view. Front and side share world X (= −vertex.y,
      // the same mapping foldedFront uses), so this projector runs HORIZONTALLY at
      // constant world x, tying the side view to the front view — not the top view.
