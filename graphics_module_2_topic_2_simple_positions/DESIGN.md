...
- **Disabled state:** rendered at reduced opacity with a small padlock icon at the row's end, so it reads as "locked for now," not broken (e.g. controls are inert on the empty start, before a solid is added).
