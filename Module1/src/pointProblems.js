...
// `target` with a ±0.5 mm tolerance; ONLY the keys present in `target` are checked, so a
// distance the textbook does not pin is left free.
//
// Values are in millimetres (the Points sim's display unit; 1 world unit = 10 mm, harmonised
// with Module 2; sliders read 0–100 mm). Targets stay inside the slider range. `tier` groups the
// cards in the library overlay and orders the difficulty ramp: first-quadrant basics → the other
// three quadrants → points lying on a plane.
//
