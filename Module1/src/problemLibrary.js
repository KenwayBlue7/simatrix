// problemLibrary.js — Module 1 Problem Library controller (a leaf layer).
//
// A quiet practice mode that wraps the Guided Stepper WITHOUT touching it. A "Practice
// problems" entry opens a full-viewport, focus-trapped overlay of textbook problems (grouped
// by tier); picking one routes through the single reset path, pins the statement in the wizard
// panel, jumps the learner to the step where the setup can be dialled, and runs a gentle
// self-check. NOTHING is auto-filled — the student dials the setup by hand (the pedagogy is the
// word-problem → 3D translation), and the check lights green only when the live config matches.
//
// Generic by design: it is DATA-DRIVEN through the `problems` config (list / tiers / fieldLabels
// / entryStep), so ONE controller serves both the Points and the Lines sims. The self-check is
// driven by `sim.onStateChange`, which the engine fires at the end of every rebuild() — the one
// seam every parameter and step change passes through.
//
// Layering (CLAUDE.md): imports nothing from the orchestrator; it speaks only to the injected
// `sim` facade and the passed `problems` data. Uses window.simAPI.pause/resume (via the engine)
// while the overlay covers the live viewport.

const TOL = 0.5; // ±0.5 cm / ° tolerance on every numeric target field
...
export function initProblemLibrary(sim, problems) {
  ...
  const completeBtn = $('problem-complete');
  ...
  /** Reveal the "Complete & next problem" button only once the check is green. */
  function showComplete(on) {
    if (completeBtn) completeBtn.hidden = !on;
  }
  ...
  function loadProblem(problem) {
    ...
    // entryStepFor reads a single target's shape (Lines keys off `.case`), so hand it one
    // representative object — the first alternative when the target is an OR-array.
  ...
  function completeAndNext() {
    sim.toast?.('Solved — nice work!');
    sim.announce('Problem solved. Choose another from the library.');
    exitProblem();
    openOverlay();
  }
  ...
  completeBtn?.addEventListener('click', completeAndNext, listen);
